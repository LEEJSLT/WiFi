...
def wifiConnect(pwd):
    ...
    print("const iface disconnect", const.IFACE_DISCONNECTED)
    if wifistatus == const.IFACE_INACTIVE:
        # 创建WiFi连接文件
        print ("Jajajajajajajajajajajaja")
        profile = pywifi.Profile()
        # 要连接WiFi的名称
        profile.ssid = "LightHouse"
        # 网卡的开放状态
    ...
    while True:
        try:
            ...
            if bool:
                ...
                # 跳出当前循环，进行下一次循环
                print("正在破解 SSID 为 %s 的 WIFI密码，当前校验的密码为：%s"%("LightHouse",pwd))
        except:
            continue
    end = datetime.datetime.now()
    if(success):
